...
class GTLayer(nn.Module):
    ...
    def forward(self, adj, embeds):
        ...
        qEmbeds = (rowEmbeds @ self.qTrans).view(
            [-1, args.head, args.latdim // args.head]
        )
        kEmbeds = (colEmbeds @ self.kTrans).view(
            [-1, args.head, args.latdim // args.head]
        )
        vEmbeds = (colEmbeds @ self.vTrans).view(
            [-1, args.head, args.latdim // args.head]
        )
...
class LocalGraph(nn.Module):
    ...
    def forward(self, allOneAdj, embeds):
        ...
        scores = scores.reshape((score.shape[0],1))
        residual_transform = ResidualTransform(scores.shape[-1])
        scores = residual_transform(scores)
...
class ResidualTransform(nn.Module):
    def __init__(self,input_dim):
        ...
        self.l0=nn.Linear(input_dim,10*input_dim)
        self.l1=nn.Linear(10*input_dim,input_dim)

    def forward(self, x):
        h = F.relu(self.l0(x))
        h = F.relu(self.l1(h))
        return x + t.sigmoid(h)
    ...
    def forward(self, adj, seeds):
        ...
        if self.flag == False:
            ...
            print(
                ...
                "%.2f" % (tem.shape[0] / (args.user + args.item)),
                tem.shape[0],
                (args.user + args.item),
            )
        ...
        if self.flag == False:
            print(
                ...
                "%.2f" % (maskNodes.shape[0] / (args.user + args.item)),
                maskNodes.shape[0],
                (args.user + args.item),
            )
            self.flag = True
        ...
        encoderAdj = self.normalizeAdj(
            t.sparse.FloatTensor(
                t.stack([rows, cols], dim=0), t.ones_like(rows), adj.shape
            )
        )
        ...
        decoderAdj = t.sparse.FloatTensor(
            t.stack([newRows, newCols], dim=0),
            t.ones_like(newRows).float(),
            adj.shape,
        )
